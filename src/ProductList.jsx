...
function ProductList({ onHomeClick }) {
  ...
  const handleAddToCart = (product) => {
    ...
    if (existingItem) {
      dispatch(addItem({ ...product, quantity: existingItem.quantity + 1 })); // Update quantity if item exists
    } else {
      dispatch(addItem({ ...product, quantity: 1 })); // Add new item to cart
    }
  };

  const getItemQuantity = (productName) => {
    const item = cart.find((item) => item.name === productName);
    return item ? item.quantity : 0; // Return quantity if item exists, otherwise 0
  };
              ...
              <div className="product-list">
                {category.plants.map((plant, plantIndex) => (
                  <div className="product-card" key={plantIndex}>
                    ...
                    <span>
                      Quantity in Cart: {getItemQuantity(plant.name)}
                    </span>
                  </div>
                ))}
              </div>
